            ...
            <TracingBeam>
                <ul className="flex flex-col gap-4">
                    {data["what-is-ai"].events.map((event, i) => {
                        return (
                            <motion.a key={i} href={event.wiki_link} variants={data["what-is-ai"].eventsVariants} initial='closed' whileInView='open' whileHover='hover' transition={{ duration: 1, type: 'spring' }} className="group relative">
                                <p className="text-green-300 font-bold tracking-wider w-fit group-hover:-translate-x-2 transition delay-75">{event.year}</p>
                                <div className="flex">
